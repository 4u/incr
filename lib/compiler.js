...
Compiler.prototype.visitTag = function(tag) {
  ...
  var openAtrrs = (attrs.key || 'null') + ', ' + this.statics(attrs);

  if (attrs.dynamic || tag.attributeBlocks.length) {
    this.ipush('elementOpenStart', '"' + tag.name + '", ' + openAtrrs);
    this.dynamics(attrs);
    this.visitAttributeBlocks(tag.attributeBlocks.slice());
    this.ipush('elementOpenEnd');
  } else {
    this.ipush('elementOpen', '"' + tag.name + '", ' + openAtrrs);
  }
...
Compiler.prototype.visitCode = function(code) {
  if (code.buffer) {
    ...
    // All text nodes already esacped
    ...
    attributeBlocks.forEach(function(block) {
      try {
        ...
        for (var name in json) {
          this.ipush('attr', '"' + name + '", ' + JSON.stringify(json[name]));
        }
      } catch(ex) {
        var name = uid();
        this.push('var ' + name + ' = ' + block + ';');
        var key = uid();
        this.push(format('for (var %s in %s) {' +
          format(this.templates.incremental, 'attr', key + ', ' + name + '[' + key + ']') +
        '}', key, name));
      }
    }, this);
  }
...
Compiler.prototype.registerMixin = function(mixin) {
  ...
  var funcName = this.getMixinFunctionName(mixin);

  this.push(`function ${funcName}(${args}) {`);
  this.visit(mixin.block);
  this.push(`}`);
  this.push('');
};

Compiler.prototype.getMixinFunctionName = function(mixin) {
  return '_' + mixin.name + 'Mixin';
}

Compiler.prototype.callMixin = function(mixin) {
  if (this.mixins.indexOf(mixin.name) === -1) {
    throw errorAtNode(mixin, `Non-existent mixin: ${mixin.name}`);
  }

  var funcName = this.getMixinFunctionName(mixin);
  var valuesString = mixin.args ? `, ${mixin.args}` : '';

  if (mixin.block) {
    this.push(`${funcName}(function() {`);
    this.visit(mixin.block);
    this.push(`}${valuesString});`);
  } else {
    var funcCall = mixin.args 
        ? `${funcName}(undefined${valuesString});`
        : `${funcName}();`;
    this.push(funcCall);
  }
}
...
Compiler.prototype.visitMixin = function(mixin) {
  if (mixin.call) {
    this.callMixin(mixin);
  } else {
    this.registerMixin(mixin);
  }
};

Compiler.prototype.visitMixinBlock = function(mixinBlock) {
  this.push('block();');
};
...
Compiler.prototype.attrsToArray = function(attrs) {
  var ret = [];
...
Compiler.prototype.statics = function(attrs) {
  if (attrs && attrs.dynamic) {
    return 'null';
  }

  attrs = this.attrsToArray(attrs);
  if (attrs.length === 0) {
    return 'null';
  }

  var statics = attrs.reduce(function(rv, item) {
...
Compiler.prototype.dynamics = function(attrs) {
  if (attrs && !attrs.dynamic) {
    return;
  }

  this.attrsToArray(attrs).forEach(function(item) {
    this.ipush('attr', '"' + item[0] + '", ' + item[1]);
  }, this);
};
