...
Compiler.prototype.getMixin = function(mixin) {
  var storedMixin = this.mixins[mixin.name];
  if (!storedMixin) {
    throw errorAtNode(mixin, `Undefined mixin: ${mixin.name} at ${mixin.filename}:${mixin.line}`);
  }
...
Compiler.prototype.visitMixin = function(mixin) {
  if (mixin.call) {
    var storedMixin = this.getMixin(mixin);

    if (mixin.block) {
      this.contextBlocks.push(mixin.block);
    }

    var args = ['block'];
    if (storedMixin.args) {
      args.push(storedMixin.args);
    }

    var values = [!!mixin.block];
    if (mixin.args) {
      values.push(mixin.args);
    }

    this.push(`(function(${args.join(', ')}) {`);
    this.visit(storedMixin.block);
...
Compiler.prototype.visitMixinBlock = function(mixinBlock) {
  var contextBlock = this.contextBlocks.pop();
  if (contextBlock) {
    this.visit(contextBlock);
  }
};
